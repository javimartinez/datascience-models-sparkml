...
/**
  ...
  */
@Since("1.4.0")
class StringIndexerCustom @Since("1.4.0")(
  ...
  @Since("1.4.1")
  override def copy(extra: ParamMap): StringIndexerCustom = defaultCopy(extra)
}

@Since("1.6.0")
object StringIndexerCustom extends DefaultParamsReadable[StringIndexerCustom] {

  @Since("1.6.0")
  override def load(path: String): StringIndexerCustom = super.load(path)
}

/**
  * Model fitted by [[StringIndexerCustom]].
  *
...
class StringIndexerModel(
  ...
  override def transform(dataset: Dataset[_]): DataFrame = {
    ...
    val indexs = labelToIndex.toArray.map(_._2)

    val metadata = new org.apache.spark.ml.attribute.NumericAttribute(name = Some($(outputCol)), min = Some(indexs.min), max = Some(indexs.max)).toMetadata()
